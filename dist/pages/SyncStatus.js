...
function SyncStatus() {
    return (<div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <MetricCard_1.default title="Active Syncs" value={mockSyncMetrics.activeSyncs} icon={lucide_react_1.RefreshCw} status="healthy"/>
        
        <MetricCard_1.default title="Avg Sync Time" value={`${mockSyncMetrics.averageSyncTime}s`} icon={lucide_react_1.Clock} status="healthy" change={{ value: 8, type: 'decrease', period: 'last week' }}/>
        
        <MetricCard_1.default title="Success Rate" value={`${mockSyncMetrics.successRate}%`} icon={lucide_react_1.CheckCircle} status="healthy" change={{ value: 0.5, type: 'increase', period: 'last week' }}/>
      </div>
      ...
      <div className="card">
        ...
        <recharts_1.ResponsiveContainer width="100%" height={300}>
          <recharts_1.LineChart data={mockSyncLagData}>
            <recharts_1.CartesianGrid strokeDasharray="3 3" className="opacity-30"/>
            <recharts_1.XAxis dataKey="time" className="text-xs"/>
            <recharts_1.YAxis className="text-xs" label={{ value: 'Seconds', angle: -90, position: 'insideLeft' }}/>
            <recharts_1.Tooltip contentStyle={{
            backgroundColor: 'rgba(255, 255, 255, 0.95)',
            border: '1px solid #e2e8f0',
            borderRadius: '8px'
        }}/>
            <recharts_1.Line type="monotone" dataKey="lag" stroke="#f59e0b" strokeWidth={2} dot={{ fill: '#f59e0b', strokeWidth: 2, r: 4 }}/>
      ...
      <div className="card">
        ...
        <div className="space-y-4">
          {mockActiveSyncs.map((sync) => (<div key={sync.id} className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-lg">
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mb-2">
                <div className="bg-blue-500 h-2 rounded-full transition-all duration-300" style={{ width: `${sync.progress}%` }}/>
              </div>
      ...
      <div className="card">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {mockSyncHistory.map((sync) => (<tr key={sync.id} className="border-b border-gray-200/30 dark:border-gray-700/30 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <StatusBadge_1.default status={sync.status}/>
                  </td>
